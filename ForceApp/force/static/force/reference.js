...
function detectWalls() {
    ...
    // Bottom wall
    if (player.y + player.h > 1500) {
        player.y = 1500 - player.h;
    }
}
...
function init() {
    ...
    ctx.clearRect(0, 1500, canvas.width + 100, 1500 + 100);
    // console.log(scale)
    ...
    for (var i = 0; i < 800; i++) {
        ...
        var y = Math.max(1500+radius, 1500 + Math.random() * (canvas.height - radius) - 1500);
...
function animate() {
    ctx.clearRect(0, 1500, canvas.width, canvas.height);
    ctx.fillStyle = '#74BDCB';
    ctx.fillRect(0, 1500, canvas.width, canvas.height);
